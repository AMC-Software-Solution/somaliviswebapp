...
import { IPaymentTransaction, defaultValue } from 'app/shared/model/payment-transaction.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IPaymentTransaction>,
  entity: defaultValue,
  updating: false,
  totalItems: 0,
  updateSuccess: false,
...
export type PaymentTransactionState = Readonly<typeof initialState>;

// Reducer

export default (state: PaymentTransactionState = initialState, action): PaymentTransactionState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IPaymentTransaction> = (page, size, sort) => {
  const requestUrl = `${apiUrl}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_PAYMENTTRANSACTION_LIST,
    payload: axios.get<IPaymentTransaction>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};

export const getEntity: ICrudGetAction<IPaymentTransaction> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_PAYMENTTRANSACTION,
    payload: axios.get<IPaymentTransaction>(requestUrl),
  };
};

export const createEntity: ICrudPutAction<IPaymentTransaction> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IPaymentTransaction> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IPaymentTransaction> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
