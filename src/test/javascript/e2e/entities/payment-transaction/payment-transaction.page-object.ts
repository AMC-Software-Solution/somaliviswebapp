...
import PaymentTransactionUpdatePage from './payment-transaction-update.page-object';

const expect = chai.expect;
export class PaymentTransactionDeleteDialog {
  deleteModal = element(by.className('modal'));
  private dialogTitle: ElementFinder = element(by.id('somaliviswebappApp.paymentTransaction.delete.question'));
...
export default class PaymentTransactionComponentsPage {
  ...
  title: ElementFinder = element(by.id('payment-transaction-heading'));
  ...
  async goToPage(navBarPage: NavBarPage) {
    await navBarPage.getEntityPage('payment-transaction');
  ...
  async deletePaymentTransaction() {
    ...
    expect(await paymentTransactionDeleteDialog.getDialogTitle().getAttribute('id')).to.match(
      /somaliviswebappApp.paymentTransaction.delete.question/
    );
